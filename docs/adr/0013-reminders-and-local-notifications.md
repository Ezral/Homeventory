...
- **Android expanded shade:** a composed 2:1 PNG card (`1080×540`) is attached as `BigPictureStyle`. The left third is the linked item or room photo (moss placeholder if none); title and body sit on the right two-thirds. Photos are downloaded at **schedule/sync time** and baked into the notification so signed Storage URLs can expire before the alarm fires. Collapsed shade still uses the system title/body row (`flutter_local_notifications` has no custom RemoteViews). `largeIcon` is omitted so Android does not pin the photo on the right.
